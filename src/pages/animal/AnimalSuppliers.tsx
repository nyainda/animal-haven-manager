...
interface SupplierCardProps {
  supplier: Supplier;
  animalId: string;
  onEdit: (supplierId: string) => void; // Changed to string
...
const SupplierCard: React.FC<SupplierCardProps> = ({ supplier, animalId, onEdit, onDelete }) => {
  ...
  const { borderClass: importanceBorderClass, iconColorClass: importanceIconColor } = getImportanceStyles(supplier.supplier_importance);
  const statusStyle = getStatusBadgeStyle(supplier.status);

  const handleEditClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    console.log('[SUPPLIER] Edit clicked, supplier ID:', supplier.id);
    if (supplier.id) onEdit(supplier.id);
  };

  const handleDeleteClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    console.log('[SUPPLIER] Delete clicked, supplier ID:', supplier.id);
    onDelete(supplier);
  };
  ...
  const renderMetricBadge = (icon: React.ReactNode, value: number | string, label: string, variant: string = 'secondary') => (
    <Tooltip>
      <TooltipTrigger>
        <Badge variant={variant as any} className="text-xs font-medium flex items-center">
          {icon}
          {value}
        </Badge>
      </TooltipTrigger>
  ...
  return (
    <Card
      className={`group relative overflow-hidden shadow-sm hover:shadow-lg transition-all duration-200 ease-in-out flex flex-col h-full ${importanceBorderClass} ${
        isExpired ? 'border-t-destructive/60 border-r-destructive/60 border-b-destructive/60 bg-destructive/5 dark:bg-destructive/10' : 'border-border'
      }`}
    >
        ...
        <CardDescription className="text-xs text-muted-foreground flex items-center gap-x-3 gap-y-1 flex-wrap">
          <Tooltip>
            <TooltipTrigger className="flex items-center cursor-default">
              ...
              <span>
                {formatDateTime(supplier.contract_start_date)} - {formatDateTime(supplier.contract_end_date)}
              </span>
            </TooltipTrigger>
            <TooltipContent side="bottom">Contract Period</TooltipContent>
          </Tooltip>
      ...
      <CardContent className="p-4 pt-2 pb-3 flex-grow">
        <div className="flex flex-wrap gap-2 mb-3">
          ...
          {supplier.supplier_rating !== undefined && (
            renderMetricBadge(<Star className="h-3 w-3 mr-1.5" />, supplier.supplier_rating, 'Supplier Rating', 'default')
          )}
        </div>

        <div className="mb-3">
          <p
            className={`text-sm text-foreground/90 whitespace-pre-wrap ${!supplier.notes ? 'text-muted-foreground italic' : ''} ${
        ...
        <div className="text-xs text-muted-foreground mb-4">
          <h4 className="font-semibold text-sm text-foreground mb-2">Key Information</h4>
          <p>
            <strong>Business Reg:</strong> {supplier.business_registration_number || 'N/A'}
          </p>
          <p>
            <strong>Contact:</strong>{' '}
            {supplier.contact?.name || supplier.contact_name || 'N/A'} (
            {supplier.contact?.position || supplier.contact_position || 'No position'})
          </p>
          <p>
            <strong>Email:</strong>{' '}
            {supplier.contact?.email || supplier.contact_email || 'N/A'}
          </p>
          <p>
            <strong>Phone:</strong>{' '}
            {supplier.contact?.phone || supplier.contact_phone || 'N/A'}
          </p>
          <p>
        ...
        <Button
          variant="ghost"
          size="sm"
          className="w-full text-xs text-primary flex items-center justify-between"
          onClick={() => setIsDetailsExpanded(!isDetailsExpanded)}
        >
          <span>{isDetailsExpanded ? 'Hide Additional Details' : 'Show Additional Details'}</span>
          {isDetailsExpanded ? <ArrowUp className="h-4 w-4" /> : <ArrowDown className="h-4 w-4" />}
        </Button>

        {isDetailsExpanded && (
          <div className="mt-3 text-xs text-muted-foreground space-y-4">
            <div>
              <h4 className="font-semibold text-sm text-foreground mb-2">Financial Details</h4>
              <div className="grid grid-cols-2 gap-2">
                <p>
                  <strong>Tax Number:</strong> {supplier.tax_number || 'N/A'}
                </p>
                <p>
                  <strong>Credit Limit:</strong>{' '}
                  {supplier.credit_limit ? `${supplier.currency} ${supplier.credit_limit.toLocaleString()}` : 'N/A'}
                </p>
                <p>
                  <strong>Tax Rate:</strong> {supplier.tax_rate ? `${supplier.tax_rate}%` : 'N/A'}
                </p>
                <p>
                  <strong>Payment Terms:</strong> {supplier.payment_terms || 'N/A'}
                </p>
                <p>
                  <strong>Bank:</strong> {supplier.bank_name || 'N/A'} {supplier.bank_branch ? `(${supplier.bank_branch})` : ''}
                </p>
                <p>
                  <strong>Account:</strong> {supplier.account_number || 'N/A'}
                </p>
                <p>
                  <strong>SWIFT:</strong> {supplier.swift_code || 'N/A'}
                </p>
                <p>
                  <strong>IBAN:</strong> {supplier.iban || 'N/A'}
                </p>
              </div>
            </div>

            <div>
              <h4 className="font-semibold text-sm text-foreground mb-2">Inventory Metrics</h4>
              <div className="flex flex-wrap gap-2 mb-2">
                {renderMetricBadge(
                  <Package className="h-3 w-3 mr-1.5" />,
                  supplier.inventory_level,
                  'Current Inventory Level',
                  supplier.inventory_level <= supplier.reorder_point ? 'destructive' : 'secondary'
                )}
                {renderMetricBadge(<Package className="h-3 w-3 mr-1.5" />, supplier.reorder_point, 'Reorder Point')}
                {renderMetricBadge(<Package className="h-3 w-3 mr-1.5" />, supplier.minimum_order_quantity, 'Minimum Order Quantity')}
                {renderMetricBadge(<Clock className="h-3 w-3 mr-1.5" />, `${supplier.lead_time_days} days`, 'Lead Time')}
              </div>
            </div>

            <div>
              <h4 className="font-semibold text-sm text-foreground mb-2">Other Details</h4>
              <p>
                <strong>Shop:</strong> {supplier.shop_name || 'N/A'}
              </p>
              <p>
                <strong>Product Type:</strong> {supplier.product_type || 'N/A'}
              </p>
              <p>
...
const AnimalSuppliers: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        const [animalData, suppliersData] = await Promise.all([fetchAnimal(id), fetchSuppliers(id)]);
        if (isMounted) {
          setAnimal(animalData);
          setSuppliers(suppliersData.sort((a, b) => parseISO(b.created_at).getTime() - parseISO(a.created_at).getTime()));
          console.log('[SUPPLIER] Loaded suppliers:', suppliersData);
        }
      } catch (error) {
  ...
  const handleEditSupplier = useCallback((supplierId: string) => {
    if (!id || !supplierId) {
      console.log('[SUPPLIER] Edit blocked: Missing ID', { animalId: id, supplierId });
      toast.error('Cannot edit supplier: Missing ID');
      return;
    }
    console.log('[SUPPLIER] Navigating to edit:', `/animals/${id}/suppliers/${supplierId}/edit`);
      ...
      <div className="flex items-center justify-center min-h-[calc(100vh-150px)] px-4">
        <Card className="w-full max-w-md text-center shadow-lg border-destructive/50">
          <CardHeader>
            ...
            <CardDescription>We couldn't find the animal associated with this ID.</CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-muted-foreground mb-6">Please check the ID or navigate back to the animals list.</p>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                {loading && !animal ? (
                  ...
                  <>
                    <h1 className="text-2xl font-bold tracking-tight text-foreground">{animal?.name}'s Suppliers</h1>
                    <p className="text-sm text-muted-foreground">View and manage suppliers for {animal?.name}.</p>
                  </>
                )}
              </div>
            </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid grid-cols-2 sm:grid-cols-4 gap-2 w-full sm:w-auto mb-6 h-auto p-1.5">
            <TabsTrigger value="all" className="py-1.5 text-xs sm:text-sm">
              All <Badge variant="secondary" className="ml-1.5 px-1.5">{supplierCounts.all}</Badge>
            </TabsTrigger>
            <TabsTrigger value="active" className="py-1.5 text-xs sm:text-sm">
              Active <Badge variant="secondary" className="ml-1.5 px-1.5">{supplierCounts.active}</Badge>
            </TabsTrigger>
            <TabsTrigger value="inactive" className="py-1.5 text-xs sm:text-sm">
              Inactive <Badge variant="secondary" className="ml-1.5 px-1.5">{supplierCounts.inactive}</Badge>
            </TabsTrigger>
            <TabsTrigger value="expired" className="py-1.5 text-xs sm:text-sm">
              Expired{' '}
              <Badge variant={supplierCounts.expired > 0 ? 'destructive' : 'secondary'} className="ml-1.5 px-1.5">
                {supplierCounts.expired}
              </Badge>
            </TabsTrigger>
          </TabsList>
